"""
Runpod serverless handler for Text/LLM generation service.
...
def get_client() -> OpenAI:
    """Initialize and return OpenAI-compatible client (singleton pattern)."""
    global _client
    if _client is None:
        _client = OpenAI(
        ...
        logger.info(f"Initialized OpenAI client with base_url={MODEL_BASE_URL}")
    return _client
...
def handler(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    {
        "input": {...},
        "headers": {...}  # Optional, for API key validation
    }
    """
